...
A regression test checks that the UI checkbox set exactly matches `_SEARCHERS`, so adding a future search adapter without exposing it in the UI causes the test suite to fail.
...
- Auth headers/cookies are not blindly forwarded to arbitrary CDN hosts.
- Localhost/private literal IP download targets are rejected.
...
The plugin currently includes regression tests for authentication, catalog adapters, import flow, multi-file selection, portal checkbox coverage and Speed Dial window reuse.
...
The v0.3.3 validation run passed:

- Python compile check
- embedded JavaScript syntax check
- **42/42 tests**, repeated twice
- Pyright: **0 errors, 0 warnings**

Set:

```text
SEARCH_ENGINE_AUTORUN=1
```

to make the plugin open its search window automatically during development.
...
- [`CHANGELOG_0.3.3.md`](CHANGELOG_0.3.3.md) — per-portal search checkboxes and persisted source selection.
- [`CHANGELOG_0.3.2.md`](CHANGELOG_0.3.2.md) — current-project import and multi-file checkbox selection.
- [`README_AUTHENTICATED_IMPORT.md`](README_AUTHENTICATED_IMPORT.md) — authenticated import implementation notes.
