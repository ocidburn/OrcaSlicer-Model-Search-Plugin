...
SPEC = importlib.util.spec_from_file_location("search_engine", os.path.join(HERE, "search_engine.py"))
...
class AuthStoreTests(unittest.TestCase):
    ...
    def test_token_normalization_accepts_copied_headers(self):
        self.assertEqual(mod.AuthManager.normalize_token("makerworld", "Bearer abc"), "abc")
        self.assertEqual(mod.AuthManager.normalize_token("nexprint", "foo=1; auth_token=NX123; bar=2"), "NX123")
        self.assertEqual(mod.AuthManager.normalize_token("makeronline", "XX-Token: AC123"), "AC123")
    ...
    def test_auth_headers_are_not_sent_to_external_cdn(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            api_headers = auth._request_headers("makerworld", "https://api.bambulab.com/v1/foo")
...
class ResolverTests(unittest.TestCase):
    def test_makerworld_uses_internal_model_id_and_profile_download_endpoint(self):
        ...
        auth = FakeAuth("makerworld", [FakeResponse(data=design), FakeResponse(data=signed)])
        model = {
            "platform": "MakerWorld",
            "_model_id": 3183685,
    ...
    def test_makerworld_selects_first_instance_when_url_has_no_profile(self):
        ...
        auth = FakeAuth("makerworld", [FakeResponse(data=design), FakeResponse(data=instances), FakeResponse(data=signed)])
        model = {"platform": "MakerWorld", "_model_id": 10, "url": "https://makerworld.com/en/models/10"}
        files = mod.MakerWorldSearcher.get_files(model, auth)
    ...
    def test_nexprint_lists_all_model_files(self):
        payload = {"code": 0, "data": {"modelFileInfoList": [
        ...
        files = mod.NexprintSearcher.get_files({"_model_id": 42, "url": "https://www.nexprint.com/models/42"}, auth)
    ...
    def test_makeronline_lists_all_model_files(self):
        payload = {"code": 0, "data": {"files": [
            {"file_name": "part.step", "url": "https://cdn.makeronline.com/part.step"},
            {"file_name": "part.stl", "url": "https://cdn.makeronline.com/part.stl"},
        ]}}
    ...
    def test_import_token_from_slicer_config(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            with open(config, "w", encoding="utf-8") as fh:
                json.dump({"anycubic_cloud": {"access_token": "1234567890abcdefTOKEN"}}, fh)
            auth = mod.AuthManager(mod.AuthStore(os.path.join(td, "sessions.json")))
